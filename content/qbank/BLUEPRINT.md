...
| `acute_neuro_change` | Recognizing a new neurologic change on trends | 6 | B | Evolving asymmetry of ischemic stroke and its trend correlates (falling ADR/total power on the affected side, relative asymmetry deflection, narrowing aEEG margins); hemorrhage and evolving mass effect; rising intracranial pressure and herniation; vasospasm after subarachnoid hemorrhage; new focal slowing after cardiac or neurosurgery; how early the change is identifiable and which trend declares it first. Terminology for what the record looks like belongs in `background_terminology`; outcome evidence belongs in `clinical_prognosis` — this domain is about noticing a change as it happens. |
