...
- Lab export worker computes the existing viewer sidecar, reads raw windows from the exported EDF/LAY, and renders those samples plus the **actual sidecar arrays**. It stores QA PNGs alongside the export. QA images use a shared left/right spectrogram scale. Raw windows currently cover start, midpoint and90%; event-directed sampling is not implemented. A screen can only pass within its documented coverage.
...
1. Apply `supabase/migrations/20260925000001_eeg_visual_qa.sql` through the existing migration workflow; verify RLS and service-role access. No migration was applied in this session.
2. Configure worker-side `EEG_QA_BASE_URL`, `EEG_QA_API_KEY`, `EEG_QA_MODEL` for a verified vision-capable OpenAI-compatible provider. Never expose these through `NEXT_PUBLIC_*`. Provider timeout120s, image maximum8, output limit3000 tokens. The existing Bifrost credential returned an empty model list during discovery; no live provider QA call has been verified.
